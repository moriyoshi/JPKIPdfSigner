 ...
 */
public class JPKIPdfSignatureAppearance {
    ...
    private JPKIPdfStamperImp writer;
    ...
    private JPKIPdfStamper stamper;
    private boolean preClosed = false;
    private JPKIPdfSigGenericPKCS sigStandard;
    private int range[];
    ...
    */
     ...
     * @param jpki JPKI wrapper
     * @param filter the cryptographic filter type. It can be SELF_SIGNED, VERISIGN_SIGNED or WINCER_SIGNED
     */
    public void setCrypto(JPKIWrapper jpki, PdfName filter) throws JPKIWrapperException {
        this.jpki = jpki;
        this.certChain = new X509Certificate[] { jpki.getCertificate() };
        this.filter = filter;
    }
    ...
    public PdfTemplate getAppearance() throws DocumentException {
        ...
        if (app[2] == null) {
            ...
            case NAME_AND_DESCRIPTION:
                String signedBy = JPKIPdfPKCS7.getSubjectFields(certChain[0]).getField("CN");
    ...
    public JPKIWrapper getJPKIWrapper() {
        return jpki;
    }
    ...
    public void preClose(HashMap<PdfName, Integer> exclusionSizes) throws IOException, DocumentException {
        ...
        if (cryptoDictionary == null) {
            if (PdfName.ADOBE_PPKLITE.equals(getFilter()))
                sigStandard = new JPKIPdfSigGenericPKCS.PPKLite(getProvider());
            else if (PdfName.ADOBE_PPKMS.equals(getFilter()))
                sigStandard = new JPKIPdfSigGenericPKCS.PPKMS(getProvider());
    ...
    public void close(PdfDictionary update) throws IOException, DocumentException {
        try {
            ...
            try { sigStandard.close(); } catch (Exception e) {}
        }
    }
    ...
    public InputStream getRangeStream() {
        return new JPKIPdfSignatureAppearance.RangeStream(raf, bout, range);
    }
     ...
     */
    public JPKIPdfStamper getStamper() {
        return stamper;
    }

    void setStamper(JPKIPdfStamper stamper) {
     ...
     */
    public JPKIPdfSigGenericPKCS getSigStandard() {
        return sigStandard;
    }
